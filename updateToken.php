...
elseif (empty($data) || !isset($data->mobile) || empty(trim($data->mobile))):
  ...
  try {
    ...
    $fetch_user_stmt->bindValue(':mobile', $data->mobile, PDO::PARAM_STR);
    ...
    if ($fetch_user_stmt->rowCount()):
      ...
      $returnData = $error_handler->getResponse(1, 200, 'Mobile found!', $user);
        ...
        $token =  $data->token;
        $update = "UPDATE `users` SET `token`=:token WHERE `mobile`=:mobile";
        $update_stmt = $conn->prepare($update);
        $update_stmt->bindValue(':mobile', $data->mobile, PDO::PARAM_STR);
        $update_stmt->bindValue(':token', $token, PDO::PARAM_STR);
        if ($update_stmt->execute()):
          $returnData = $error_handler->getResponse(1, 200, 'Token updated successfully!');
        else:
